...
현재 소스(`AStar_PathFinding/`)는 최적화가 모두 반영된 최종본입니다. 각 최적화 단계의 "이전" 코드는 별도 폴더가 아니라 단계별 측정 결과(`길찾기 측정 자료/`)로만 남아 있어, 변경 전 코드는 최종본의 흔적(스테일 주석 등)과 폴더명으로 추정합니다. 추정 지점은 각 흐름의 "확인 필요"에 표시했습니다. 다만 휴리스틱만은 예외로, 비교 측정을 위해 옥타일과 유클리드 두 버전이 현재 코드에 모두 남아 있습니다(9번 흐름).
...
- 현재 구조: `PathFinding` 한 번의 호출이 **같은 맵·같은 시작/도착점에 대해 A*를 두 번** 돈다. 먼저 옥타일 h로(`Search`) 돌고, `PathClear`로 결과 격자와 오픈리스트를 비운 뒤 유클리드 h로(`Search2`) 한 번 더 돈다. 휴리스틱 A/B 비교(9번 흐름)를 위한 구조이며, 두 구간은 각각 별도 프로파일러 태그로 측정된다.
...
- 관련 함수: `PathFinding`, `Search`(A* 옥타일), `Search2`(A* 유클리드), `PathClear`, `PriorityQueue::push/pop/top/empty`
- 처리 순서:
...
- 확장 노드 수 집계: 두 구간이 각각 `visitcount1`(옥타일) / `visitcount2`(유클리드)로 실제 확장한(= 8방향을 펼친) 노드 수를 세어 `m_octile` / `m_euclid`(`Result` 구조체)에 누적한다. 지연 삭제로 건너뛴 노드는 세지 않는다.
...
- 목적: 8방향 이동 비용에 맞는 휴리스틱으로 바꿔 탐색 노드 수를 줄인다.
- 관련 파일: `PathFinder.cpp`(`PathFinding`의 시작 f, `Search`의 h 계산), `PathFinder.h`(Tile 주석)
- 관련 클래스: `PathFinder`
- 관련 함수: `PathFinding`, `Search`
- 처리 순서:
  1. 기존: 초기엔 유클리드 거리 휴리스틱(두 점 사이의 직선 거리)을 사용했다.
  2. 변경 후: h와 시작 f를 8방향 이동(직선 비용 1, 대각 비용 1.5)에 맞는 옥타일 형태 `(dx+dy) - 0.5*min(dx,dy)`로 계산한다. 즉 `(dx+dy)`에서 대각으로 줄어드는 만큼을 빼는 방식이다.
  3. 이 식은 장애물이 없을 때의 실제 최소 비용과 일치한다. 대각으로 min(dx,dy)번(1.5씩), 직선으로 max-min번(1씩) 이동한 합이 곧 max + 0.5*min 이기 때문이다.
  4. 현재 코드에는 두 식이 모두 남아 있다. 옥타일은 `Search`(와 첫 구간의 시작 f), 유클리드는 `Search2`(와 둘째 구간의 시작 f)에 들어 있고 나머지 로직은 동일하다. 
- 최적화 포인트: 유클리드는 격자 제약이 없는 직선 거리라 모든 노드에서 h값이 옥타일 이하로 추정되고 목적지가 멀수록 비례해 h가 크게 하락한다. f = g + h가 먼 노드일수록 더 많이 깎이므로 우선순위 큐에서 먼 노드가 실제보다 앞서 뽑히고, 탐색이 옆으로 퍼져 확장 노드가 늘어난다.
...
- 처리 순서:
  ...
  2. 변경 후(A* 버전): 방향 오프셋 배열(`Search`는 `dir_x`/`dir_y`, `Search2`는 `dir_x2`/`dir_y2`)과 비용 배열(`cost`)을 static으로 두고, `x = xpos + dir_x[dir]`, `cost[dir]`로 분기 없이 계산한다. static이라 함수 호출마다 배열을 다시 초기화하지 않는다.
...
- 관련 파일: `ProfilerTLS.h`(`CProfiler`, `CProfilerManager`), `AppController.cpp`(`OnAutoPathFinding`, `Init`), `PathFinder.cpp`(측정 구간), 결과 `Profiling [...]_*.txt`
...
- 처리 순서:
  ...
  2. A* 측정 구간에 `CProfiler` 객체를 생성한다(RAII). 생성자가 시작 시각(성능 카운터)을 기록하고, 소멸자가 경과를 태그별 데이터에 누적한다. 태그는 두 개이며(`PathFinding_Octile`, `PathFinding_Euclid`) 각각 옥타일 구간과 유클리드 구간을 잰다.
  3. 태그별로 총 시간·호출 수와 함께 상위 몇 개·하위 몇 개의 극단값을 유지한다.
  4. 출력 시 극단값을 제외한 평균을 마이크로초로 계산해 파일로 기록한다. 파일명에는 두 휴리스틱의 누적 확장 노드 수가 함께 들어간다(예: `Profiling [PathFinding_Euclid 37823399_Octile 31352137]_20260826_21.30.59.txt`).
...
- 목적: 휴리스틱 변경의 효과가 "무엇 때문에" 나왔는지를, 시간뿐 아니라 확장 노드 수로 분리해 확인한다.
- 관련 파일: `PathFinder.cpp`(`PathFinding`의 두 구간), `PathFinder.h`(`Result` 구조체, `m_octile`/`m_euclid`), `AppController.cpp`(`OnAutoPathFinding`의 결과 저장)
- 관련 클래스: `PathFinder`, `PathFinder::Result`
- 관련 함수: `PathFinding`, `Search`, `Search2`, `PathClear`
- 처리 순서:
  1. `ValidateGoal` 통과 후 첫 구간에서 옥타일 h로 A*를 돌고, `visitcount1`을 `m_octile`에 누적한다.
  2. `PathClear`로 결과 격자와 오픈리스트를 비운다. 맵과 시작/도착점은 그대로 둔다.
  3. 둘째 구간에서 유클리드 h로 같은 문제를 다시 풀고, `visitcount2`를 `m_euclid`에 누적한다.
  4. 5만 회가 끝나면 각 `Result`에서 최대·최소 1건씩을 빼고(극단값 제외) 누적 확장 노드 수를 파일명에 기록한다.
- 측정 설계상의 의미: 맵·시작점·도착점이 완전히 같은 **짝지은 비교**라, 맵 난이도 편차가 두 휴리스틱에 동일하게 반영된다. 다만 옥타일이 항상 먼저 돌므로 캐시 상태는 유클리드 쪽에 유리하다.
- 관련 테스트: `길찾기 측정 자료/측정 자료/기본/휴리스틱 알고리즘 변경/Profiling [PathFinding_Euclid 37823399_Octile 31352137]_20260826_21.30.59.txt`
- 결과 해석은 `docs/Test_Report.md` 5장 참고.
